$(function()
{
    ...
    jQuery(document).scroll(function(sidebar)
    {
    ...
    jQuery("form:not(.not-api)").submit(function()
    {
        ...
        if (action.indexOf('/api/user/logout') > -1) {
            $('input[name="auth_token"]').val('');
            $.ajaxSetup({headers: { 'X-AuthToken': '' }});
        ...
        var options = {
            ...
            {
                if (res.result && res.result.api_token) {
                    $('input[name="api_token"]').val(res.result.api_token);
                    $.ajaxSetup({headers: { 'X-AuthToken': res.result.api_token }});
                }

                if (res.result && res.result.socket_token) {
                    connectSocket(res.result.socket_token);
                }
...
/**
 * Connect to web socket with Socket IO
 *
 * @param token
 */
function connectSocket(token)
{
    if (typeof io !== 'undefined') {
        var host = app.socket.url;

        console.log(host);

        // Establish connection
        window.socket = io(host, {query: {token: token}});
        // Listen to channel
        window.socket.on(token, function(message)
        {
            console.log(message);
            $('.socket-console').prepend($('<div>' + json2string(message) + '</div>'));
        });
    }
}

$(function() {
    var $form = $('.connect-to-socket');
    $form.on('submit', function(e){
        e.preventDefault();
        connectSocket($form.find('[name="socket_token"]').val());
    });
});
